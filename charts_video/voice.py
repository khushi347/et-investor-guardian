...
def generate_guardian_audio(text, filename="briefing.mp3"):
    ...
    """
    try:
        # 1. Clean up old briefing files to keep the folder light
        folder = os.path.dirname(__file__)
        
        # 2. Professional 'Guardian' Tone (Australian English)
        tts = gTTS(text=text, lang='en', tld='com.au')
        
        # 3. Dynamic Filename: Prevents MoviePy overwrite crashes
        unique_name = f"voice_{int(time.time())}.mp3"
        output_path = os.path.join(folder, unique_name)
        
        tts.save(output_path)
        
        # 4. Automatic Cleanup of older mp3 files (Optional but recommended)
