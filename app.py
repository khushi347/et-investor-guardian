...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        if data_dict:
            ...
            st.session_state.portfolio_dict = data_dict # Save for AI context
    ...
    if "portfolio_df" in st.session_state:
        ...
        current_portfolio = {"RELIANCE.NS": 15, "TCS.NS": 10}
...
# Define the Master Ticker for this session
...
with left:
    ...
    st.markdown(f"<h3 style='margin-bottom:10px; color:#E6F1FF'>{target_ticker.replace('.NS', '')} Signal</h3>", unsafe_allow_html=True)
    ...
    if fig:
        ...
        st.error(f"Failed to load chart for {target}. Check your internet connection.")
    if fig:
        fig.update_layout(
        ...
        # FIX: Make the background almost solid dark blue/black
        paper_bgcolor='rgba(11, 19, 43, 0.95)', 
        plot_bgcolor='rgba(11, 19, 43, 1.0)', 
        
        # Add a border around the chart image so it looks like a UI panel
...
with right:
    ...
    prompt = st.chat_input("Ask Guardian...")
    if prompt:
        st.session_state.messages.append({"role": "user", "content": prompt})
        with st.spinner("Processing..."):
            advice_chat = generate_advice(target_ticker, portfolio=current_portfolio)
            full_reply = f"**{advice_chat.get('decision', 'SIGNAL DETECTED')}**\n\n{advice_chat.get('reason', 'Analysis ready.')}"
            st.session_state.messages.append({"role": "assistant", "content": full_reply})
        st.rerun()

    # --- VIDEO BRIEFING SECTION ---
    st.markdown("---")
    st.markdown('<p class="section-header">Multimodal Briefing</p>', unsafe_allow_html=True)
    
    if st.button("🛡️ Generate Video Briefing", use_container_width=True):
        with st.status("Initializing Guardian Veo Engine...", expanded=True) as status:
            st.write("🧠 Analyzing portfolio context...")
            briefing_advice = generate_advice(target_ticker, portfolio=current_portfolio)
            
            st.write("🎬 Rendering cinematic briefing...")
            # We use target_ticker which is already 'fixed' with .NS
            video_path = generate_briefing_video(target_ticker, briefing_advice)
            
            status.update(label="Briefing Secured!", state="complete")
        
        if os.path.exists(video_path):
            st.video(video_path)
            with open(video_path, "rb") as file:
                st.download_button("📥 Save Briefing", data=file, file_name=video_path, use_container_width=True)